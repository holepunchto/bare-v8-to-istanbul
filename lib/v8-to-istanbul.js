...
const { dirname, isAbsolute, join, resolve } = require(isBare ? 'bare-path' : 'path')
const { fileURLToPath } = require(isBare ? 'bare-url' : 'url')
...
const { readFileSync } = require(isBare ? 'bare-fs' : 'fs')
const process = require(isBare ? 'bare-process' : 'process')

let readFile = () => { throw compatError }
try {
  readFile = require(isBare ? 'bare-fs' : 'fs').promises.readFile
} catch (_err) {
  // most likely we're on an older version of Node.js.
}
const { TraceMap } = require('@jridgewell/trace-mapping')
const isOlderNode10 = isNode && /^v10\.(([0-9]\.)|(1[0-5]\.))/u.test(process.version)
const isNode8 = isNode && /^v8\./.test(process.version)
  ...
  async sourcesContentFromSources () {
    const fileList = this.sourceMap.sources.map(relativePath => {
      ...
      return readFile(realPath, 'utf-8')
        .then(result => result)
        .catch(err => {
          console.error(`failed to load ${realPath}: ${err.message}`)
        })
    })
